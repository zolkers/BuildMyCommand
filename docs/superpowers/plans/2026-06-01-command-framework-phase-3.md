...
- [ ] Write failing tests for registering `Commands.literal("ban")...build()` through `registry.register(node)`.
- [ ] Write failing tests for manual API validation: nonblank names, duplicate child/alias/flag rejection, required arguments before optional arguments, one greedy argument, greedy argument last, and missing option values during dispatch.
- [ ] Implement immutable public specs/factories for literals, required/optional/greedy arguments, boolean flags, value options through `Flags.option(...)`, aliases, children, and handlers using the existing public `CommandRegistry.CommandExecutor` contract.
- [ ] Compile public `CommandNode` instances into the existing internal registry model.
- [ ] Verify manual, builder, route DSL, and annotation registration still produce equivalent dispatch/help/schema behavior for representative nested literals, aliases, required arguments, greedy optional arguments, boolean flags, value options, help, and schema.
